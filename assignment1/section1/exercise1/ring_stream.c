...
void execute_mpi_ring(int numprocs, FILE *fptr)
{
	...
	for (int iteration = 1; iteration <= numprocs; iteration++)
	{
		...
		if (iteration == 1)
		{
			...
			MPI_Send(&send_left_buf, 1, MPI_INT, left_neighbour, my_rank * 10, ring_communicator);

			//send message to right neighbour
			send_right_buf = -1 * my_rank;
			MPI_Send(&send_right_buf, 1, MPI_INT, right_neighbour, my_rank * 10, ring_communicator);
		}
		else
		{
			//send message to left neighbour with right tag
			send_left_buf = msg_right_buf - my_rank;
			MPI_Send(&send_left_buf, 1, MPI_INT, left_neighbour, msg_right_status.MPI_TAG, ring_communicator);

			//send message to right neighbour with right tag
			send_right_buf = msg_left_buf + my_rank;
			MPI_Send(&send_right_buf, 1, MPI_INT, right_neighbour, msg_left_status.MPI_TAG, ring_communicator);
		}

		//receive message from right neighbour
		MPI_Recv(&msg_right_buf, 1, MPI_INT, right_neighbour, msg_right_tag, ring_communicator, &msg_right_status);
		counter_msg++;

		//receive message from left neighbour
		MPI_Recv(&msg_left_buf, 1, MPI_INT, left_neighbour, msg_left_tag, ring_communicator, &msg_left_status);
		counter_msg++;
	}
