...
for j in tcp mlx shm 
		do for iteration in {1..5}
			do 
				filename="core-$j"
				mkdir -p "gpu/csv/intel/$filename"
			
				echo "mpiexec -n 2 -genv I_MPI_PIN_PROCESSOR_LIST=0,2 -genv I_MPI_FABRICS ofi -genv I_MPI_OFI_PROVIDER $j ./IMB-MPI1 PingPong -msglog 29" > "gpu/csv/intel/$filename/$filename-$iteration".csv
				echo "$1" >> "gpu/csv/intel/$filename/$filename-$iteration".csv 
				echo "lamba, bandwith computed by fitting data" >> "gpu/csv/intel/$filename/$filename-$iteration".csv  
				mpiexec -n 2 -genv I_MPI_PIN_PROCESSOR_LIST=0,2 -genv I_MPI_FABRICS ofi -genv I_MPI_OFI_PROVIDER $j ~/mpi-bench/mpi-benchmarks/src_c/IMB-MPI1 PingPong -msglog 29 2>/dev/null | grep -v ^\# | grep -v '^$' | tr -s ' ' | sed 's/  */,/g' | cut -c 2- >> "gpu/csv/intel/$filename/$filename-$iteration".csv
...
for j in tcp mlx shm
		do for iteration in {1..5}
			do 
				...
				echo "mpiexec -n 2 -genv I_MPI_PIN_PROCESSOR_LIST=0,1 -genv I_MPI_FABRICS ofi -genv I_MPI_OFI_PROVIDER $j ./IMB-MPI1 PingPong -msglog 29" > "gpu/csv/intel/$filename/$filename-$iteration".csv 
				echo "$1" >> "gpu/csv/intel/$filename/$filename-$iteration".csv 
				echo "lamba, bandwith computed by fitting data" >> "gpu/csv/intel/$filename/$filename-$iteration".csv 
				mpiexec -n 2 -genv I_MPI_PIN_PROCESSOR_LIST=0,1 -genv I_MPI_FABRICS ofi -genv I_MPI_OFI_PROVIDER $j ~/mpi-bench/mpi-benchmarks/src_c/IMB-MPI1 PingPong -msglog 29 2>/dev/null | grep -v ^\# | grep -v '^$' | tr -s ' ' | sed 's/  */,/g' | cut -c 2- >> "gpu/csv/intel/$filename/$filename-$iteration".csv 
				echo "$filename-$iteration"
			done
		done
...
for j in tcp mlx
		do for iteration in {1..5}
			do 
				...
				echo "mpiexec -n 2 -ppn 1 -genv I_MPI_FABRICS ofi -genv I_MPI_OFI_PROVIDER $j ./IMB-MPI1 PingPong -msglog 29" > "gpu/csv/intel/$filename/$filename-$iteration".csv 
				echo "$1, $2" >> "gpu/csv/intel/$filename/$filename-$iteration".csv 
				echo "lamba, bandwith computed by fitting data" >> "gpu/csv/intel/$filename/$filename-$iteration".csv 
				mpiexec -n 2 -ppn 1 -genv I_MPI_FABRICS ofi -genv I_MPI_OFI_PROVIDER $j ~/mpi-bench/mpi-benchmarks/src_c/IMB-MPI1 PingPong -msglog 29 2>/dev/null | grep -v ^\# | grep -v '^$' | tr -s ' ' | sed 's/  */,/g' | cut -c 2- >> "gpu/csv/intel/$filename/$filename-$iteration".csv 
				echo "$filename-$iteration"
			done
		done
