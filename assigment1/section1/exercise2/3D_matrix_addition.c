...
int main(int argc, char *argv[])
{
	if (argc != 7)
	{
		printf("Usage: mpirun -np 'number_of_processors' %s 'x_dimension' 'y_dimension' 'z_dimension' 'x_topology_dimension' 'y_topology_dimension' 'z_topology_dimension'", argv[0]);
		return 1;
	}
	...
	int ndims = 3;
	int dims[3] = {x_topo_dim, y_topo_dim, z_topo_dim};
	int periods[3] = {0, 0, 0};
	int reorder = 1;
